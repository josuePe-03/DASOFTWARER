<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold text-white">Editar Permiso</h2>
    </x-slot>
        ...
        <form action="{{ route('permisos.update', $permiso) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="descripcion" rows="4"
                          class="w-full rounded-lg bg-white/20 text-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:border-cyan-300">{{ $permiso->descripcion }}</textarea>
            </div>
